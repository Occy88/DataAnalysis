...
merge_cluster <- function(cluster, measure, n_attr) {
  ...
  if (length(c1) >= length(c2)) {
    ...
    cluster[[ min_rc[[2]]]] <- c(c1, c2)
    return(list(cluster[-min_rc[[1]]], unlist(rev(min_rc)), min(d)))
  }
...
my_hclust <- function(clusters, measure, n_attr) {
  ...
  return(list(clusters, combination_matrix,order))
}
